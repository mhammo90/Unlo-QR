	...
	constructor(a, b, c, d, f) {
		...
		requestAnimationFrame(() => {
			...
			h &&
				...
				(a.style.width = "0"),
				(a.style.height = "0"),
				this.$overlay && this.$overlay.parentElement && this.$overlay.parentElement.removeChild(this.$overlay),
				delete this.$overlay,
	...
	static async listCameras(a = !1) {
		...
		try {
			a &&
				...
				(c = await navigator.mediaDevices.getUserMedia({ audio: !1, video: !0 }));
		} catch (d) {}
		try {
	...
	async pause(a = !1) {
		...
		let b = () => {
			this.$video.srcObject instanceof MediaStream &&
				(e._stopVideoStream(this.$video.srcObject), (this.$video.srcObject = null));
		};
		if (a) return b(), !0;
				...
				q = await new Promise((l, v) => {
					...
					u = (t) => {
						t.data.id === y &&
							...
							clearTimeout(w),
							null !== t.data.data
								? l({ data: t.data.data, cornerPoints: e._convertPoints(t.data.cornerPoints, m) })
						...
						try {
							...
							if (/not implemented|service unavailable/.test(g))
								return (
									...
									e.scanImage(a, { scanRegion: m, canvas: d, disallowCanvasResizing: f, alsoTryWithoutScanRegion: h })
								);
	...
	static async createQrEngine(a) {
		...
		return b &&
			...
			(await b
				...
				.then(({ architecture: c, platformVersion: d }) => /arm/i.test(c || "arm") && 13 <= parseInt(d || "13"))
				.catch(() => !0))
			? a()
			...
			if (this.$overlay) {
				...
				switch (p) {
					...
					case "fill":
						...
						break;
					default:
						("cover" === p ? k > q : k < q) ? ((l = f), (g = l * k)) : ((g = d), (l = g / k)),
							"scale-down" === p && ((g = Math.min(g, b)), (l = Math.min(l, c)));
				}
			...
			("requestVideoFrameCallback" in this.$video
				...
				if (!(1 >= this.$video.readyState)) {
					...
					!e._disableBarcodeDetector ||
						...
						(this._qrEnginePromise = e.createQrEngine());
					c
						? (this._onDecode ? this._onDecode(c) : this._legacyOnDecode && this._legacyOnDecode(c.data),
			...
			try {
				let f = await navigator.mediaDevices.getUserMedia({ video: d, audio: !1 }),
					h =
						...
						(d.facingMode ? this._preferredCamera : "environment" === this._preferredCamera ? "user" : "environment");
